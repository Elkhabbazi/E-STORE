...
$categoriesHommes = ['Jabador', 'Jellaba Homme', 'Chaussures Homme'];

// Requête principale — produits hommes
$where  = ["category IN ('Jabador','Jellaba Homme','Chaussures Homme')", "stock > 0"];
$params = [];

if ($category !== '') {
    if ($category === 'Chaussures Homme') {
        // Chaussures hommes = Chaussures Homme seulement
        $where  = ["category IN ('Chaussures Homme')", "stock > 0"];
        $params = [];
    } elseif ($category === 'Jabador') {
        $where  = ["category IN ('Jabador','Jabador Homme')", "stock > 0"];
        $params = [];
    } else {
...
$occasions = $conn->query("
    ...
    AND category IN ('Jabador','Jellaba Homme','Chaussures Homme')
    ORDER BY occasion
...
<div class="page-header">
  ...
  <div class="page-header-inner">
    ...
    <p class="page-sub">Jabador · Jellaba Homme · Chaussures Homme</p>
  </div>
</div>
    ...
    <div class="products-grid">
      <?php if (empty($products)): ?>
        <div class="empty">
          ...
          <a href="hommes.php" style="display:inline-block;margin-top:1rem;font-size:.65rem;letter-spacing:.15em;text-transform:uppercase;color:var(--gold);">
